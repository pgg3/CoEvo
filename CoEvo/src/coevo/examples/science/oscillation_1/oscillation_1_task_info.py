...
class Oscillation1TaskInfo(ProgramTaskInfo):
    def __init__(self, **kwargs):
        ...
        task_info = \
            ...
            f"system with driving force, given data on position, and velocity. "
        program_template = \
            f'```python\n' \
            f'import numpy as np\n' \
            f'def equation(x: np.ndarray, v: np.ndarray, params: np.ndarray) -> np.ndarray:\n'\
            ...
            f'    """\n' \
            f'    dv = params[0] * x  +  params[1] * v +  + params[3]  # Example equation\n' \
            f'    return dv\n'\
            f'```\n'
        super().__init__(
            task_info=task_info,
