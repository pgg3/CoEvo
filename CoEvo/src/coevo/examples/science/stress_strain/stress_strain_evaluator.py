...
class StressStrainEvaluator(ProgramEvaluator):
    ...
    def evaluate_task(self, code_string, return_parameters=False, **kwargs):
        try:
            ...
            with warnings.catch_warnings():
                ...
                heuristic_module = importlib.import_module("coevo.examples.science.stress_strain.temp_module")
                eva = importlib.reload(heuristic_module)

                start_time = time.time()
                from scipy.optimize import minimize
                def loss(params):
                    y_pred = eva.equation(self.evaluate_data["strain"], self.evaluate_data["temp"], params)
                    return np.mean((y_pred - self.evaluate_data["stress"]) ** 2)
    ...
    def _evaluate_with_fixed_params(self,code_string, params):
        ...
        with warnings.catch_warnings():
            ...
            return  [np.mean((y_pred - self.evaluate_data["stress"]) ** 2), time.time() - start_time]